...
```python
git dff <commit-id>
#after looking at the commits, we found some creds inside the second commit : 

-response = requests.get('https://api.craft.htb/api/auth/login',  auth=('dinesh', '4aUh0A8PbVJxgd'), verify=False)

#we didn't find any other useful information in the other commits.
```
...
## Trying to login via ssh as dinesh
- failed on port 22
- lets try port 6022 as well : 
- but it only accepts public key authentication. password authentication is not supported.