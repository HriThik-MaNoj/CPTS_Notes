...
- we also found some credentials in the output
![[Pasted image 20251230100754.png]]
- tom NMds732Js2761
- Let's try sshing into our target using these credentials..
- ![[Pasted image 20251230100922.png]]
- Oops.. I think we need the ssh key to login.. let's try forcing the password authentication..
![[Pasted image 20251230101050.png]]
- Still no luck..
- Let's try these credentials with the mail servers..
- Enumerating the mail box: 
![[Pasted image 20251230103157.png]]
- So we selected inbox..and let's read the email present in it..
- ![[Pasted image 20251230103255.png]]
- Seem's like we got the ssh private key.. lets save it and connect to the server via ssh..
- ![[Pasted image 20251230103456.png]]
- Ohhh Yeah.. we're in baby..!!
- when I checked the .bash_history file.. i saw the commands they used to connect to a mysql server.. so  tried to do the same..
- ![[Pasted image 20251230103742.png]]
- Woho.. we're in mysql.. now all we gotta do is to find the passsword for the user HTB
- I used `show databases;` command to see all the databases, selected the database `users` in which we had a `users table`, and found the `password for HTB inside `that.
- 