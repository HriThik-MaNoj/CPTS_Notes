...
- The information that we have obtained using `rpcclient `can also be obtained using other tools. For example, the `SMBMap` and `CrackMapExec` tools are also widely used and helpful for the enumeration of SMB services.
#### smbmap
```
smbmap -H <ip-address>
```
