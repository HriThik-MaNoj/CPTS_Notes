...
Techniques to bypass security mechanisms when scanning protected networks.
...
Using the Decoy (`-D`) scanning method, we can generate random (`RND`) a specific number (for example 5) of IP addresses separated by a colon (:). Our real IP address is then randomly placed between the generated IP addresses.

> **Critical:** Our decoys must be alive, otherwise our target may be unreachable due to SYN-flooding security mechanisms.
...
### Bypassing Firewalls (The Source Port Trick)

This is the most "hacky" part. It describes a trick to get past a Firewall or IDS (Intrusion Detection System).
