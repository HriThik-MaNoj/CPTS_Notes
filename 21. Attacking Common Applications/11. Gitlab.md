...
- Go to `/explore` to see if there are any interesting public projects
- Try creating an account and then explore
...
```python
bash gitlab_user_enum.sh --url http://gitlab.inlanefreight.local:8081 --userlist /usr/share/seclists/Usernames/xato-net-10-million-usernames.txt | grep exists!
```

- We can use this [exploit](https://www.exploit-db.com/exploits/49951) to achieve RCE. in some older version
- Its an authenticated RCE, however if we encounter a vulnerable version of GitLab that allows for self-registration, we can sign up for an account and pull of the attack :
