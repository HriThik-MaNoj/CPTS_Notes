...
![Email from Bryan Robinson to Sarah McDonald about the start of an external penetration test for Inlanefreight. Testing traffic from IP 10.10.14.15. Secondary contact: John Lee, Principal Security Consultant. Includes contact details and testing procedure overview.](https://academy.hackthebox.com/storage/modules/163/start_testing.png)
...
![Web page showing JobEntry portal with 'Find The Perfect Job That You Deserved' banner, search options for jobs and talents, and login page for vpn.inlanefreight.local.](https://academy.hackthebox.com/storage/modules/163/ilfreight_eyewitness.png)
...
![Web page titled 'Test' with a search bar and message 'Welcome to Test'. No front page content created. Includes a link to the User Guide. Powered by Drupal.](https://academy.hackthebox.com/storage/modules/163/drupal.png)
...
![HTTP request to dev.inlanefreight.local with TRACK method, response includes X-Custom-IP-Authorization: 172.18.0.1.](https://academy.hackthebox.com/storage/modules/163/track_method.png)
...
![HTTP request to dev.inlanefreight.local using TRACK method, with response showing X-Custom-IP-Authorization: 127.0.0.1. HTML response includes a form for file upload.](https://academy.hackthebox.com/storage/modules/163/track_method_auth.png)
...
![Web page titled 'Pixel Shop' with a colorful camera logo, options for Crop, Filters, and Advance Filters, and buttons to Browse and Submit.](https://academy.hackthebox.com/storage/modules/163/pixel_shop.png)
...
![Web page for support.inlanefreight.local with a Cookie Editor open, showing a session cookie with value fcfaf93ab169bc943b92109f0a845d99.](https://academy.hackthebox.com/storage/modules/163/edit_cookie.png)
...
![Cargo tracking page for order ID 934342477703467000, estimated delivery on 2022-8-5 by BLUEDART. Status: Not Verified. Tracking #: TESTING THIS. Progress shows 'Order confirmed' with images of shipping stages.](https://academy.hackthebox.com/storage/modules/163/tracking_rendered.png)
...
![Document viewer displaying a list of system users and paths, including root, daemon, and various service accounts, with details like shell paths and user IDs.](https://academy.hackthebox.com/storage/modules/163/ssrf_file_read.png)
...
![GitLab projects page showing 'shopdev2.inlanefreight.local' and 'Monitoring' projects under 'Explore projects' tab, with options to filter by name and updated date.](https://academy.hackthebox.com/storage/modules/163/gitlab_explore.png)
...
![HTTP request to shopdev2.inlanefreight.local with POST method to /checkout.php, including XML payload. Response shows server user list from /etc/passwd.](https://academy.hackthebox.com/storage/modules/163/xxe_passwd.png)
...
![Terminal displaying a list of commands: ls, cat, whoami, date, help, clear, reboot, cd, mv, rm, rmdir, touch, connection_test.](https://academy.hackthebox.com/storage/modules/163/monitoring.png)
...
![Terminal showing files: todo.txt, note.txt, contact.txt. Contents of todo.txt: tasks to remove staging files and configure authentication. Contents of note.txt: reminder to configure authentication service and test applications in development mode. Contents of contact.txt: admin@inlanefreight.local.](https://academy.hackthebox.com/storage/modules/163/monitoring_files.png)
...
![HTTP GET request to monitoring.inlanefreight.local with command injection attempt using socat. Response shows successful ping to 127.0.0.1 with /usr/bin/socat output.](https://academy.hackthebox.com/storage/modules/163/socat_check.png)
...
![Firefox connection settings showing manual proxy configuration with SOCKS Host set to 127.0.0.1, port 8081, and SOCKS v5 selected.](https://academy.hackthebox.com/storage/modules/163/ff_socks.png)


Browsing to the page confirms our suspicions.

![Web page for DNN installation with the message 'Every journey begins with the first step.' Offers options for using DNN in the cloud, on a server, or desktop. Features a logo with 'Open, Scalable, Extensible, Secure.](https://academy.hackthebox.com/storage/modules/163/dnn_homepage.png)
...
![User management interface showing two users: 'dev inlanefreight' with email dev01@inlanefreight and 'SuperUser Account' with email admin@inlanefreight, both joined on 06/01/2022. Options to remove deleted users and add a new user.](https://academy.hackthebox.com/storage/modules/163/dnn_logged_in.png)
...
![Web page showing server information for 172.16.8.20 with port 80, running Microsoft-IIS/10.0. Lists privileges like SeAssignPrimaryTokenPrivilege and SeImpersonatePrivilege, with their states.](https://academy.hackthebox.com/storage/modules/163/asp_webshell.png)
...
![File management interface on 172.16.8.20 showing a dialog to open SECURITY.SAVE (48.0 KB). Options to cancel or save the file. File list includes nc.exe, PrintSpoofer64.exe, and shell.asp.](https://academy.hackthebox.com/storage/modules/163/download_sam.png)
...
![Diagram showing user HPORTER@INLANEFREIGHT.LOCAL with local admin, execution, and outbound control rights. Connection to SSMALLS@INLANEFREIGHT.LOCAL via ForceChangePassword.](https://academy.hackthebox.com/storage/modules/163/hporter.png)
...
![Diagram showing DOMAIN USERS@INLANEFREIGHT.LOCAL with group membership and local admin rights. Connection to ACADEMY-AEN-DEV01.INLANEFREIGHT.LOCAL via CanRDP.](https://academy.hackthebox.com/storage/modules/163/all_rdp.png)
...
![Diagram showing MSSQLADM@INLANEFREIGHT.LOCAL with execution and control rights, connected to TTIMMONS@INLANEFREIGHT.LOCAL via GenericWrite.](https://academy.hackthebox.com/storage/modules/163/ttimmons.png)
...
![Diagram showing TTIMMONS@INLANEFREIGHT.LOCAL with execution and control rights, connected to SERVER ADMINS@INLANEFREIGHT.LOCAL via GenericAll.](https://academy.hackthebox.com/storage/modules/163/ttimmons_server_admins.png)
...
![Diagram showing SERVER ADMINS@INLANEFREIGHT.LOCAL with execution and control rights, connected to INLANEFREIGHT.LOCAL via GetChanges and GetChangesAll.](https://academy.hackthebox.com/storage/modules/163/dcsync.png)
...
![Email from Bryan Robinson about completed external penetration test for Inlanefreight, listing vulnerabilities like command injection and weak passwords.](https://academy.hackthebox.com/storage/modules/163/end_testing.png)
...
![GIF showcasing a Concept Review list.](https://academy.hackthebox.com/storage/modules/163/ConceptReviewList.gif)
...
![GIF showcasing the Global Search Usage in HackTheBox Academy.](https://academy.hackthebox.com/storage/modules/163/GlobalSearchUsage.gif)
...
![GIF showcasing Starting Point on HackTheBox Labs.](https://academy.hackthebox.com/storage/modules/163/QuickStartingPointTour.gif) 
...
![GIF showcasing the Tracks on HackTheBox Labs.](https://academy.hackthebox.com/storage/modules/163/QuickTracksTour.gif)
...
![GIF showcasing the Pro Labs on HackTheBox Labs.](https://academy.hackthebox.com/storage/modules/163/QuickProLabsTour.gif)
