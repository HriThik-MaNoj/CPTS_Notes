...
<div class="card bg-light">
    <div class="card-body">
        <p class="mb-0">Tip: If you prefer to use to a dark theme, you may do so in Burp by going to (<code>Burp>Settings>User interface>Display</code>) and selecting "dark" under (<code>theme</code>), and in ZAP by going to (<code>Tools>Options>Display</code>) and selecting "Flat Dark" in (<code>Look and Feel</code>).</p>
    </div>
</div>
...
<div class="card bg-light">
    <div class="card-body">
        <p class="mb-0"><b>Note:</b> In case we wanted to serve the web proxy on a different port, we can do that in Burp under (<code>Proxy>Proxy settings>Proxy listeners</code>), or in ZAP under (<code>Tools>Options>Network>Local Servers/Proxies</code>). In both cases, we must ensure that the proxy configured in Firefox uses the same port.</p>
    </div>
</div>
...
<div class="card bg-light">
    <div class="card-body">
        <p class="mb-0">Note: This configuration is already added to FoxyProxy in PwnBox, so you don't have to do this step if you are using PwnBox.</p>
    </div>
</div>
...
As we modify and send custom HTTP requests, we may have to perform various types of encoding and decoding to interact with the web server properly. Both tools have built-in encoders that can help us in quickly encoding and decoding various types of text.
...
![Decoder tab showing encoded string and its decoded JSON output with options for text, hex, and smart decode.](https://academy.hackthebox.com/storage/modules/110/burp_b64_decode.png)
...
![Request and response details with encoded IP in request, HTML response, and Inspector showing URL and Base64 decoding.](https://academy.hackthebox.com/storage/modules/110/burp_inspector.png)
...
![Text encoding/decoding interface showing Base64 encoded string and its decoded JSON output.](https://academy.hackthebox.com/storage/modules/110/zap_b64_decode.png)
...
![Decoder tab showing JSON with admin access and its Base64 encoded string, with options for text, hex, and smart decode.](https://academy.hackthebox.com/storage/modules/110/burp_b64_encode.png)

![Text encoding interface showing JSON with admin access and its Base64 encoded output.](https://academy.hackthebox.com/storage/modules/110/zap_b64_encode.png)

<div class="card bg-light">
    <div class="card-body">
        <p class="mb-0">Tip: Burp Decoder output can be directly encoded/decoded with a different encoder. Select the new encoder method in the output pane at the bottom, and it will be encoded/decoded again. In ZAP, we can copy the output text and paste it into the input field above.</p>
    </div>
</div>
...
![Proxy tab showing HTTP GET request details with buttons: Forward, Drop, Intercept is on, Action, Open Browser.](https://academy.hackthebox.com/storage/modules/110/proxying_proxychains_curl.png)
...
![Proxy HTTP history showing GET request to /robots.txt with 404 status, request and response details displayed.](https://academy.hackthebox.com/storage/modules/110/proxying_msf.png)
...
![Intruder tab showing Attack Target configuration with host 46.101.23.188 and port 31827, option to use HTTPS.](https://academy.hackthebox.com/storage/modules/110/burp_intruder_target.png)
...
![Intruder tab showing Payload Positions with attack type Sniper, displaying HTTP GET request details.](https://academy.hackthebox.com/storage/modules/110/burp_intruder_position.png)
...
![Intruder tab showing Payload Sets configuration with payload set 1, type Simple list, and counts for payload and request.](https://academy.hackthebox.com/storage/modules/110/burp_intruder_payload_set.png)
...
![Payload Options for Simple list with file names, buttons for Paste, Load, Remove, Clear, and Add new item.](https://academy.hackthebox.com/storage/modules/110/burp_intruder_payload_wordlist.png)
...
![Payload Processing with rule 'Skip if matches [^\..*$]', buttons for Add, Edit, Remove, Up, Down.](https://academy.hackthebox.com/storage/modules/110/burp_intruder_payload_processing_2.png)

#### Payload Encoding

The fourth and final option we can apply is `Payload Encoding`, enabling us to enable or disable Payload URL-encoding.  

![Payload Encoding settings with URL-encode option for characters: ./^=<>&+?\*:;'{}|^](https://academy.hackthebox.com/storage/modules/110/burp_intruder_payload_encoding.png)
...
![Grep - Match settings to flag items with expressions. Example: '200 OK'. Options: Simple string, Regex, Case sensitive, Exclude HTTP headers.](https://academy.hackthebox.com/storage/modules/110/burp_intruder_options_match.png)
...
<div class="card bg-light">
    <div class="card-body">
        <p class="mb-0">Note: We may also use the <code>Resource Pool</code> tab on the right side vertical bar to specify how much network resources Intruder will use, which may be useful for very large attacks. For our example, we'll leave it at its default values.</p>
    </div>
</div>
...
![Results table showing requests with payloads, all with status 404, length 458.](https://academy.hackthebox.com/storage/modules/110/burp_intruder_attack_exclude.png)
...
![Results table showing requests. 'admin' payload has status 200, length 244, marked 200 OK. Others have status 404, length 458.](https://academy.hackthebox.com/storage/modules/110/burp_intruder_attack.png)
...
![Options interface for fuzzing. Retries on IO error: 3. Limit max errors: 1000. Depth First strategy selected. 20 concurrent threads. No delay in fuzzing. Follow Redirects unchecked.](https://academy.hackthebox.com/storage/modules/110/zap_fuzzer_options.png)
...
![Request and response details. Request: HTTP GET to /skills/. Response: 200 OK, includes Set-Cookie header, HTML content with 'Welcome' title.](https://academy.hackthebox.com/storage/modules/110/zap_fuzzer_dir.jpg)
...
![Sites Tree showing URLs and files: http://46.101.23.188:30873, devtools, index.php, robots.txt, sitemap.xml, wp-comments-post.php.](https://academy.hackthebox.com/storage/modules/110/zap_sites.jpg)
...
![Site Alerts: High. Remote OS Command Injection detected.](https://academy.hackthebox.com/storage/modules/110/zap_high_alert.jpg)
...
![Summary of Alerts: 1 High, 3 Medium, 8 Low, 6 Informational. Alerts include Remote OS Command Injection, Cross-Domain Misconfiguration, Directory Browsing, and others.](https://academy.hackthebox.com/storage/modules/110/zap_report.png)
...
![BApp Store showing Burp extensions. Active Scan++ highlighted, last updated 25 Mar 2021. Features include host header attacks, edge side includes, XML input handling.](https://academy.hackthebox.com/storage/modules/110/burp_bapp_store.png)
...
![BApp Store showing Burp extensions. Decoder Improved installed, last updated 19 Feb 2021. Features include encoding, decoding, hashing, tabs, Unicode support, and hex editor.](https://academy.hackthebox.com/storage/modules/110/burp_extension.png)
...
![Marketplace tab showing add-ons. Selected: FuzzDB Files and FuzzDB Offensive, both in Release status. Options to install or get more info.](https://academy.hackthebox.com/storage/modules/110/zap_fuzzdb_install.png)
...
![File Fuzzers type selected. Files listed with 'command_execution-unix.txt' checked. Payloads preview shows command execution examples.](https://academy.hackthebox.com/storage/modules/110/zap_fuzzdb_cmd_exec.png)
